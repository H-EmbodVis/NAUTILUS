...
def create_optimizer(self):
    ...
    if self.optimizer is None:
        ...
        elif self.args.nautilus_lr:
            print("nautilus optimizer init")
            optimizer_grouped_parameters = [
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if (n in decay_parameters and n not in nautilus_parameters and p.requires_grad)
                    ],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if (n not in decay_parameters and n not in nautilus_parameters and p.requires_grad)
                    ],
                    "weight_decay": 0.0,
                },
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if (n in decay_parameters and n in nautilus_parameters and p.requires_grad)
                    ],
                    "weight_decay": self.args.weight_decay,
                ...
                },
                {
                    "params": [
                        p
                        for n, p in opt_model.named_parameters()
                        if (n not in decay_parameters and n in nautilus_parameters and p.requires_grad)
                    ],
                    "weight_decay": 0.0,
                    "lr": self.args.nautilus_lr, 
                },
        ...
        if not isinstance(self.model, supported_classes):
            ...
            if getattr(self.args, 'use_lora', False):
                ...
                all_state_dict = get_peft_state_non_lora_maybe_zero_3(
                    self.model.state_dict().items(), require_grad_only = False
                )
                if self.args.local_rank == 0 or self.args.local_rank == -1:
                    self.model.config.save_pretrained(output_dir)
                    self.model.save_pretrained(output_dir, state_dict=lora_state_dict)
                    torch.save(non_lora_state_dict, os.path.join(output_dir, 'non_lora_trainables.bin'))
                    torch.save(all_state_dict, os.path.join(output_dir, WEIGHTS_NAME))
        ...
        if self.tokenizer is not None:
            ...
            if hasattr(self.tokenizer, 'chat_template'):
                with open(os.path.join(output_dir,CHAT_TEMPLATE_FILE), 'w') as f:
                    json.dump(dict(chat_template = getattr(self.tokenizer, 'chat_template')), f, indent= 2)
