...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        # Resampleing
        print(f"Resample args:{data_args.resample}")
        resample_annotations_dict = {}
        for tag, num in data_args.resample.items():
            resample_annotations = []
            for ann in list_data_dict:
                if ann['id'][1] == tag:
                    resample_annotations.extend([ann] * num)
            resample_annotations_dict[tag] = resample_annotations
            print(f"{tag} : {len(resample_annotations)}")
        for tag, r_ann in resample_annotations_dict.items():
            list_data_dict.extend(r_ann)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            # resize gt to shape of resized image range
            if sources[0]['id'][1] in ["1","2","5","7"]:
                ori_w, ori_h = get_image_size(image_file)
                input_h, input_w = grid_thw[0][1].item()*14, grid_thw[0][2].item()*14
                scale_h, scale_w = input_h / ori_h, input_w / ori_w
                sources[0]['conversations'][0]['value'] = scale_bboxes_in_text(sources[0]['conversations'][0]['value'], scale_w, scale_h)
                sources[0]['conversations'][1]['value'] = scale_bboxes_in_text(sources[0]['conversations'][1]['value'], scale_w, scale_h)
